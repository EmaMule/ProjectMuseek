...
form.addEventListener("submit", async (event) => {
  ...
  const response = await fetch(
    `https://nominatim.openstreetmap.org/search?q=${city}&city=${city},country=${country}&format=json&limit=1&accept-language='en'`
  );
  ...
  if (response.ok) {
    ...
    if (data.length === 0) {
      ...
      var displayName = data[0].display_name.toLowerCase().split(",");
      displayName = displayName.map((s) => s.replace(" ", "")); //da correggere con la prima occorrenza di " "
      ...
      if (
        ...
        const response = await fetch("../php-function/yourprofile.php", {
          method: "POST",
          body: formData,
        });
        if (response.ok) {

...
var oggetto_menu = document.getElementById("my_form");
navbarMenu.addEventListener("click", () => {
  ...
  if (navbarMenu.value == "NOT clicked") {
    ...
    var bottone_oggetto = oggetto_menu.querySelector("button");
    if (bottone_oggetto.innerHTML === "Login") {
      loginItem.innerHTML =
        '<a class="nav-link" href="../php-function/login.php">Login</a>';
    } else {
      loginItem.innerHTML =
        '<a class="nav-link" href="../php-function/no-login.php">Logout</a>';
    }
    ...
    if (loginItem.innerText === "Login" || loginItem.innerText === "Logout") {
...
searchInput.addEventListener("mouseleave", () => {
  ...
  if (searchInput.value.trim() === "") {
    ...
    document.querySelector(".search-results").style.top = "-400px";
  }
});
...
function showHint(str) {
  if (str.length == 0) {
    ...
    xmlhttp.open("GET", "../php-function/gethint.php?q=" + str, true);
    xmlhttp.send();
  }
}
